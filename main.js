...
if (handleStartupEvent()) app.exit()
...
app.on('ready', () => {
  ...
  ipc.on('settings', (e, a) => { settings = a })
  ...
  (async () => {
    try {
      const gitHubRes = await got.head(lastestURL)
      const latestTag = gitHubRes.socket._httpMessage.path.split('/').pop()
    ...
    if (updateFound === true) {
      ...
      if (process.platform === 'win32')
      {
        ...
        autoupdater.on('update-downloaded', (e, a) => {
...
function load_data(e, a) {
  fs.readFile(a.folder + slash + 'logs' + slash + 'latest.log', 'utf8', (err, t_) => {
    if (err) { e.sender.send(a.id + '_load_log', undefined); return }
    if (t_ === '' || t_ === null) return
    const data = new Array()
    const log_array = t_.split(/\r\n|\r|\n/)
    for (let i = 0; i < log_array.length; i++) {
      let e__ = ''
      const e_ = require('encoding-japanese').convert(new Buffer(log_array[i]), 'UTF-8', 'SJIS')
  ...
  fs.readFile(a.folder + slash + 'indices.ams', 'utf8', (err, t_) => {
...
function load_manage(e, a) {
  ...
  fs.readdir(a.folder + slash + 'logs', (err, files) => {
    if (files === '' || err) {
...
function backup(e, a, b) {
  fs.readdir(dir(a), (err, folders) => {
    ...
    for (let i = 0; i < folders.length; i++) {
      const r = remove[i]
      if (dir(a) + slash + r !== b) require('rimraf')(dir(a) + slash + r, error => {})
    }
    if (fs.existsSync(a.folder + slash + 'world')) copy(a.folder + slash + 'world', dir(a) + slash + time(), err => {
      if (b !== undefined) del([a.folder + slash + 'world' + slash + '**', '!' + a.folder + slash + 'world'], {force: true}).then(paths => {
        copy(b, a.folder + slash + 'world', err => {})
  ...
  if (a.mode) {
    ...
    function collect(all, already) {
      ...
      for (const i in all) {
        if (already.indexOf(i) < 0) {
          already.push(i)
          copy(dir(a.p[i], false), dir(a.p[i], true), err => { del([dir(a.p[i], false)], {force: true}).then(paths => { collect(all, already) }) })
          return
        }
      }
    }
    ...
    function back(all, already) {
      ...
      for (const i in all) {
        if (already.indexOf(i) < 0) {
          already.push(i)
          copy(dir(a.p[i], true), dir(a.p[i], false), err => { back(all, already) })
          return
        }
      }
    }
...
function read_zip(e, a) {
  let count = 0
  const list = [], all_list = [], world_list = []
  let type = '', base = ''
